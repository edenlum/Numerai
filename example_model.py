...
FEATURES = add_dir("features.json")
TRAINING = add_dir("training_data.parquet")
TOURNAMENT = add_dir(f"tournament_data_{current_round}.parquet")
VALIDATION = add_dir("validation_data.parquet")
EX_VAL_PRE = add_dir("example_validation_predictions.parquet")
...
def download_datasets():
    ...
    napi.download_dataset("numerai_training_data.parquet", TRAINING)
    napi.download_dataset("numerai_tournament_data.parquet", TOURNAMENT)
    napi.download_dataset("numerai_validation_data.parquet", VALIDATION)
    napi.download_dataset("example_validation_predictions.parquet", EX_VAL_PRE)
    napi.download_dataset("features.json", FEATURES)
...
def training_val_test(features):
    print('Reading minimal training data')
    # read in just those features along with era and target columns
    read_columns = features + [ERA_COL, DATA_TYPE_COL, TARGET_COL]
    training_data = pd.read_parquet(TRAINING, columns=read_columns)
    print('Reading minimal features of validation and tournament data...')
    validation_data = pd.read_parquet(VALIDATION, columns=read_columns)
    tournament_data = pd.read_parquet(TOURNAMENT, columns=read_columns)
    return training_data, validation_data, tournament_data
...
training_data, validation_data, tournament_data = training_val_test(features)
